...
public class Main {
    ...
    static LinkedList<String>[] tableA = new LinkedList[500];
    static LinkedList<Integer>[] tableAInt = new LinkedList[500];
    ...
    private static void put(int key, String value) {
        ...
        if (tableA[bucketValue] == null) tableA[bucketValue] = new LinkedList<>();

        // add value to bucket
        tableA[bucketValue].add(value);
    }
    ...
    private static void putInt(int key, int value) {
        ...
        // if the bucket is empty, initialize it
        if (tableAInt[bucketValue] == null) tableAInt[bucketValue] = new LinkedList<>();

        // add value to bucket
        tableAInt[bucketValue].add(value);
    }
        ...
        try {
            ...
            for (int i = 1; i < 100; i++) {
                ...
                for (int j = 0; j < 100; j++) {
                    ...
                    for(String recordA : tableA[bucketValue]){
                        // get randomV for A
                        int randomVA = getRandomV(recordA);
    ...
    private static void aggregate(int type){
        ...
        for(int i = 0; i < tableAInt.length; i++){
            ...
            if(tableAInt[i] != null) {
                for (int x : tableAInt[i]) {
                    aggregated += x;
                }

                // if we need to average, divide by bucket size
                if (type == 2) {
                    aggregated /= tableAInt[i].size();
                }
